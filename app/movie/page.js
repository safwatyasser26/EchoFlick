...
const defaultParams =
  "include_adult=true&include_video=true&vote_count.gte=500";
...
const page = () => {
  ...
  const constructUrl = () => {
    ...
    if (date.from) {
      params.append("primary_release_date.gte", date.from);
    }

    if (date.to) {
      params.append("primary_release_date.lte", date.to);
    }

    if (release) {
      params.append("primary_release_year", release);
    }
